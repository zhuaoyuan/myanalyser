...
- `src/fund_gmbd.py`：基金规模变动数据抓取（东方财富 FundArchivesDatas API，akshare 风格接口；CLI `tools/fetch_fund_gmbd.py`）
- `src/fund_cyrjg.py`：基金持有人结构数据抓取（东方财富 FundArchivesDatas API，akshare 风格接口；CLI `tools/fetch_fund_cyrjg.py`）
