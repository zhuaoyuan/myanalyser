...
```text
myanalyser/
  .python-version       # Python 版本锁定（pyenv 读取，当前 3.12.12）
  requirements.txt      # 顶层 pip 依赖（版本下限）
  requirements-lock.txt # 精确锁定（pip freeze 产出，用于复现环境）
  src/        # 业务代码与 CLI
    contracts/   # 中间产物契约定义
    validators/  # 契约校验器
    transforms/  # 流程复用转换脚本
...
- 分红/拆分校对目录：`data/versions/{run_id}/fund_etl/revised_fund_bonus_by_code`、`data/versions/{run_id}/fund_etl/revised_fund_split_by_code`
...
- `tools/fund_bonus_split_vote.py`：分红/拆分投票校对（由 fund_etl step5 完成后调用）
...
- `src/scoreboard_metrics.py`：评分榜指标计算共享模块（供 pipeline 与 verify_scoreboard_recalc 共用；与 backtest 共用 `fund_metrics_core` 保证口径一致）
- `src/fund_metrics_core.py`：基金指标计算核心逻辑（Backtest 与 Scoreboard 共用；A 股口径：243 交易日/年、20 交易日/月、回撤修复天数=峰顶到收复全程）
- `src/backtest_verify_e2e.py`：verify step10 回测适配层（ClickHouse 选基 + 本地净值 + PyBroker 引擎）
- `src/fund_gmbd.py`：基金规模变动数据抓取（东方财富 FundArchivesDatas API，akshare 风格接口；CLI `tools/prep/fetch_fund_gmbd.py`）
- `src/fund_cyrjg.py`：基金持有人结构数据抓取（东方财富 FundArchivesDatas API，akshare 风格接口；CLI `tools/prep/fetch_fund_cyrjg.py`）
- `src/compute_fund_composite_score.py`：基金综合得分计算（消费 filtered/scoreboard CSV，归一化+分组加权输出带得分的 CSV）
- `src/filter_score/`：筛选与打分模块化流水线，入口 `filter_and_score_main.py`，模板方法可扩展过滤与算分策略；内置样例：`filters/most_stable.py`（最稳健原则，从 `most_stable_logic` re-export）、`scores/low_risk_debt.py`（低风险偏债得分）
- `src/most_stable_logic.py`：最稳健原则 filter_one 共享逻辑，供 filter_score 与 backtest 共用
...
- 黑名单文件：`data/common/fund_blacklist.csv`（含 `基金代码` 列），在 **prep_data_workflow** 阶段剔除。
- 主流水线（run_full_pipeline.sh、verify.sh）以 `fund_purchase.csv` 为有效清单，不再执行 build_effective_purchase；推荐使用 prep 产出的 fund_purchase。
- `build_effective_purchase_csv.py` 仍可独立调用，供 prep 或特殊场景使用。
...
- 完整约定见 `docs/参考/v2日期与区间协议约定.md`。
- 要点：`hold_days`/`t-step`/`lookback` 均为**交易日**；1 年 = 243 交易日（A 股口径）；日期区间统一双闭 [start, end]。

## 5. 验收跑机制（E2E）

统一脚本：`tools/verify.sh`

执行顺序（12 步，含 9.5 过滤、10b 筛选打分）：
...
6. 执行 `fund_etl step2~step7`（使用 `fund_purchase.csv`，黑名单已前置到 prep）
6.5. step5 后触发分红/拆分投票校对，输出 `revised_fund_bonus_by_code` / `revised_fund_split_by_code` 并在 `data/common/revise` 留档
7. 执行复权净值计算（`adjusted_nav_tool.py`，优先使用 revised 目录）
...
10. 执行榜单入库与导出 + 2025 年回测（消费过滤后 purchase，`pipeline_scoreboard.py` 完整流程含 DB + `backtest_verify_e2e.py`）
...
10b. 执行筛选与打分（`run_filter_and_score.sh`，消费 step10 产出的 scoreboard CSV，产出 `filter_result.csv`、`scored_result.csv`）
...
1a. 准备 `fund_purchase.csv`（本地复制或 fund_etl step1；推荐使用 prep 产出）
2. 执行 `fund_etl verify + step2~step7`（使用 `fund_purchase.csv`，黑名单已前置到 prep）
