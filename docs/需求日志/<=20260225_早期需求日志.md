...
这次先不对项目文件做编辑。我想讨论一下有关myanalyser项目（/Users/zhuaoyuan/cursor-workspace/finance/myanalyser）的工程化、可维护性的问题。

## 背景
1、项目的基本背景在 README.md，注意这是一个 ai 驱动的项目。大部分时候工作流程都是：我新开一个会话，将需求通过自然语言描述给 ai（例子可参见“需求日志.md”的 QA 记录）。由ai与我讨论设计/选型/功能细节等方面疑议，我确认后 ai 开始实现并通过基本测试再交付（一般每个功能单开一个会话，平均进行3～5轮交互）。我几乎不review代码，主要通过单测和实际数据通过脚本暴露的cli验证。
2、对于不同任务，我可能会使用vscode中的openai codex插件，或 cursor 的 agent 面板。具体选择因具体场景而定。
3、当前的全局上下文文件在“项目通用约束.md”\"README.md"
4、之所以做这个讨论，是因为在当下的节点我察觉到这个项目的迭代越来越不受控制，可能存在工程上的隐患。

## 我的想法
1、目录设计。当前myanalyser目录结构有点杂乱。包括ai测试阶段临时结果目录、本地小量验证结果目录、正式结果目录、单测目录、资源文件、工具类等等没有分类。我觉得需要整理并整体做一次规范并在项目元文件规定。
2、闭环设计。“项目通用约束.md”提到需要“每次改动代码都需要设计测试用例，每一步任务可以自动化闭环验证”，我认为这是要保持的。但实践中我注意到这些单测的设计可能没有经过详细斟酌、覆盖率也存疑。另外也没有强调倾向无状态方法以支持TDD。另外并非每次改动都会强制跑全量测试。
3、伴随功能迭代的文档实时更新。目前我手动记录了“需求日志.md”，但我觉得这个方式比较原始。应该有更好的选项。
4、代码质量。怎么进一步确保，避免持续迭代过程中出现屎山代码，又避免矫枉过正、过于积极重构却影响到正常功能？
...
你这个判断很准确：现在不是“功能能不能做”，而是“系统还能不能持续安全迭代”。

结合我看到的现状（代码、测试、数据、输出、临时文件混在同层，且有 `myanalyser/myanalyser/data` 这类双层路径），建议你先做“工程护栏”，再做“结构美化”。

**1) 日常工作流建议（AI 驱动场景）**
1. 固化成 4 步会话协议（每次都一样）  
`需求澄清 -> 设计与风险 -> 实现 -> 验收证据`。  
要求 AI 在实现前先给“验收清单（可执行命令）”，不是只讲思路。
2. 强制 Definition of Done（DoD）  
每个任务交付必须包含：  
`变更文件列表`、`新增/修改测试点`、`本地执行命令`、`关键输出样例`、`回滚方式`。
3. 建立“任务级隔离”  
每个需求在独立分支或至少独立提交，避免多任务改动混杂；你不做深度 review 也能靠提交边界快速定位问题。
4. 把“全量验证”从建议改成门禁  
至少一个统一命令（例如 `make verify`）必须跑完：单测 + 核心 CLI smoke + 数据完整性检查。
...
`data/`（原始/中间/样例，按用途分）  
`artifacts/`（运行产物）  
`docs/`（README、约束、决策、变更日志）  
`tools/`（一次性脚本）  
先做“新文件按新规范放置 + 旧路径兼容”，不要一次性迁移所有历史数据。
2. 闭环设计：从“有测试”升级为“有测试策略”  
给每个模块定义测试金字塔配额：  
单元测试（纯函数、边界）> 集成测试（文件IO/依赖）> 端到端（少量关键流程）。  
重点是把计算逻辑尽量做成无状态函数，CLI 只负责编排。
3. 文档实时更新：从日志改为“决策 + 变更”双轨  
`需求日志.md` 可保留，但新增两类轻量文档：  
`docs/decisions/`（ADR，记录为什么这么做）  
`docs/changes/YYYY-MM.md`（每次功能迭代摘要+验证命令）。  
这样后续 AI 会话可以直接读“稳定上下文”，而不是长聊天记录。
4. 代码质量：用“自动化约束”替代“人工自觉”  
你不想重 review，就把质量前移到工具：  
格式化/lint/类型检查/覆盖率阈值/关键路径 smoke 测试。  
原则是“小步重构 + 行为不变 + 测试先行”，不要做无验收收益的大改。

**3) 其他可落地点子（对你这种开发模式很有用）**
1. 建一个 `TASK_TEMPLATE.md`  
每次给 AI 的需求都按模板填：目标、非目标、输入输出、验收命令、风险点。能显著降低“会话漂移”。
2. 建一个 `RUNBOOK.md`  
写清月度全量跑批流程、失败重试、结果核对、常见故障处理。未来你换模型/换工具也不怕。
3. 引入“金样本数据集”  
固定一小批基金代码作为回归基线，每次改动都跑，防止指标计算被悄悄改坏。
4. 做“代码冻结线”  
对核心计算模块设置更高门槛：改动必须附带对比报告（改前/改后关键指标）。

如果你愿意，下一步我可以直接给你一版“最小扰动”的落地方案：`两周内分 3 个阶段`，每阶段只做少量改动但明显提升可控性。
