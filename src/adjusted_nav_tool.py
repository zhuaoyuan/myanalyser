...
根据单位净值、分红、拆分/折算数据计算前复权净值，使不同时期净值可比较。

输入：净值表(净值日期, 单位净值)、分红表(除息日, 每份分红)、拆分表(拆分折算日, 拆分折算比例)。
输出：基金代码、净值日期、单位净值、复权净值、cumulative_factor。

核心逻辑：
- 分红：除息日 factor = (单位净值 + 每份分红) / 单位净值；同日多次分红累乘。
- 拆分：当日 factor = 拆分比例（如 1:2 取 2）。
- 复权净值 = 单位净值 × cumulative_factor，cumulative_factor 为当日及之前 adj_factor 的累积乘积。

allow_missing_event_until：当某分红/拆分日的净值缺失时，若该日期不晚于指定阈值，则忽略该事件，（这是一种为了保留尽可能多数据的妥协，因为离现在较远的历史数据完整度低，如果不能容忍则成立时间长的基金有相当部分会被过滤。但注意，如果使用了这种妥协，那么这份产出数据的使用场景将仅限于指定的日期之后，否则将产生严重偏差）
  当作从未发生参与计算，复权结果可能略偏低。
"""
