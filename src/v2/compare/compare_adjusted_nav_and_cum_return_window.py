...
def compare_adjusted_nav_and_cum_return_window(
    base_dir: Path,
    start_date: str,
    end_date: str,
    ...
    out_dir = output_dir or (base_dir / "fund_return_compare" / f"window_{start_date}_{end_date}")
    ...
    for code in all_codes:
        ...
        if adjusted_series.empty or cum_series.empty:
            summary_rows.append(_empty_summary_row(code, "是"))
            continue

        adjusted_series = adjusted_series[(adjusted_series.index >= start_ts) & (adjusted_series.index <= end_ts)]
        cum_series = cum_series[(cum_series.index >= start_ts) & (cum_series.index <= end_ts)]
        if adjusted_series.empty or cum_series.empty:
            summary_rows.append(_empty_summary_row(code, "是"))
        ...
        if not common_dates:
            _append_error_log(error_jsonl, code, "no common date found in window")
            summary_rows.append(_empty_summary_row(code, "是"))
            detail_df = pd.DataFrame(columns=DETAIL_COLUMNS)
            detail_df.to_csv(detail_dir / f"{code}.csv", index=False, encoding="utf-8-sig")
            continue
        ...
        scan_dates = sorted(date for date in (adjusted_dates | cum_dates) if date <= window_end)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare adjusted NAV vs cumulative return within a window.")
