...
class ComputeFundCompositeScoreTest(unittest.TestCase):
    """compute_fund_composite_score 测试用例。"""

    def test_compute_composite_score_adds_columns(self) -> None:
        """应添加得分_*、综合得分、综合排名列。"""
        df = pd.DataFrame({
            "基金代码": ["001", "002"],
            "基金名称": ["A", "B"],
    ...
    def test_compute_composite_score_ranking(self) -> None:
        """综合得分越高排名越靠前（数字越小）。"""
        df = pd.DataFrame({
            "基金代码": ["001", "002", "003"],
            "基金名称": ["A", "B", "C"],
    ...
    def test_cli_smoke(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("综合得分", out_df.columns)
            self.assertIn("综合排名", out_df.columns)
    ...
    def test_cli_missing_input_returns_nonzero(self) -> None:
        ...
        with patch(
            "sys.argv",
            ["compute_fund_composite_score", "-i", "/nonexistent/path.csv", "-o", "/tmp/out.csv"],
        ):
            code = main()
